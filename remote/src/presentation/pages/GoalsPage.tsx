...
function GoalsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: e.message || "Erro ao criar meta",
        severity: "error",
      });
    } finally {
      setLoading(false);
  ...
  const metasAtivas = goals.filter(
      ...
      <Paper sx={{ p: 3, mb: 4, borderRadius: 3 }}>
        ...
        >
          ...
          <TextField
            ...
            onChange={(e) =>
              handleFormChange("targetValue", Number(e.target.value))
            }
            required
            sx={{ flex: 1, minWidth: 120 }}
          />
          ...
          <Button
            ...
            color="primary"
            disabled={loading}
            sx={{ height: 56, alignSelf: "end", fontWeight: 700 }}
          >
            Salvar Meta
          </Button>
        </form>
      ...
      <Paper sx={{ p: 3, mb: 4, borderRadius: 3 }}>
        ...
        >
          ...
          <FormControlLabel
            control={
              <Switch
                checked={notifyOnAchieve}
                onChange={(e) => setNotifyOnAchieve(e.target.checked)}
                color="primary"
              />
            }
            ...
            {metasAtivas.map((goal) => {
              ...
              return (
                <ListItem
                  ...
                  sx={{
                    mb: 2,
                    bgcolor: progresso >= 100 ? "#e8f5e9" : "#fff",
                    borderRadius: 2,
                    boxShadow: progresso >= 100 ? 2 : 0,
                  }}
                >
                  <ListItemText
                    primary={
                      <>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          {goal.type === "vendas" ? "Vendas" : "Produção"}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          Valor da meta: {goal.targetValue} {goal.targetUnit}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          Valor atingido: {goal.currentValue} {goal.targetUnit}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          Produto:{" "}
                          {products.find((p) => p.id === goal.entityId)?.name ||
                            goal.entityId}
                        </Typography>
                      </>
                    }
                      ...
                      <>
                        <Box display="flex" alignItems="center" gap={2} mt={1}>
                          ...
                          <Typography
                            fontWeight={600}
                            color={
                              progresso >= 100 ? "success.main" : "primary"
                            }
                          >
                            {progresso}%
                          </Typography>
...
export default withLayoutAndAuth(GoalsPage);
